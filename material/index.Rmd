...
una enumeración completa, también llamada censo. Una muestra bien seleccionada de unos cuantos
...
* **Muestreo por cuotas o cupos:** Si se conocen las características de la población a estudiar, se elegirán los individuos respetando siempre ciertas cuotas por edad, género, zona de residencia, entre otras que habrán sido prefijadas.
...
Ejemplo: se desea realizar una investigación sobre el comportamiento de los padres con sus hijos. Por lo tanto, el investigador selecciona como muestra a personas que tengan hijos, ya que las considera aptas de conocimiento para formar parte de la investigación.
...
## Relación entre muestreo y machine learning

**Muestreo para conjunto de entrenamiento**

El muestreo de datos proporciona una colección de técnicas que transforman un conjunto de datos de entrenamiento para equilibrar la distribución de clases. Una vez equilibrados los datos, los algoritmos de machine learning se pueden entrenar directamente en el conjunto de datos transformado sin ninguna modificación. Esto permite abordar el desafío de la clasificación desequilibrada, incluso con distribuciones de clases severamente desequilibradas, con un método de preparación de datos.

El muestreo solo se realiza en el conjunto de datos de entrenamiento, no se realiza en al conjunto de prueba.
La razón es que la intención no es eliminar el sesgo de clase del ajuste del modelo, sino continuar evaluando el modelo resultante sobre datos que son tanto reales como representativos del dominio del problema objetivo.

**Muestreo para cartera de clientes**

Cuando la base de clientes es grande, es demasiado costoso encuestar a todos los clientes. 
En el caso de que se hagan experimentos para inferir el mejor método de retención o publicidad, 
resulta riesgoso aplicar la misma metodología a todos los clientes sin antes haber seleccionado el metodo que genera un
mejor resultado, es por eso que un diseño de muestra probabilística es la mejor opción para resolver este tipo de problemas.

La estimación del nivel de satisfacción de toda la población de clientes a través de una muestra representativa es más eficiente. Para obtener muestras representativas, se deben utilizar métodos científicos de muestreo probabilístico. Hay cuatro tipos básicos de muestreo probabilístico:
